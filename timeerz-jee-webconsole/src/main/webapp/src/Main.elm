...
update msg model =
    case msg of
        ToggleSelected id ->
            let
                newTimeerz =
                    List.map (toggle id) model.timeerz

                newToggled =
                    if List.member id model.toggled then
                        model.toggled
                    else
                        id :: model.toggled
            in
        ...
        LoadTimeerz (Ok responseStr) ->
            case decodeTimerDataList responseStr of
                Ok timeers ->
                    ( { model | timeerz = timeers }, Cmd.none )
        ...
        InputTimerId timerId ->
            if validateTimerId model timerId then
                    ...
                    errMsg =
                        case timerId of
                            ...
                            _ ->
                                "invalid ID"
                in
                ( { model | timerId = timerId, error = errMsg }, Cmd.none )

        UpdateMessage event ->
            ( { model | jobCompletions = event :: model.jobCompletions }, Cmd.none )

        NewMessage message ->
            case decodeTimerData message of
                Ok timeer ->
                    ( { model | timeerz = timeer :: model.timeerz }, Cmd.none )
    ...
    Grid.container []
        [ Grid.row []
            [ Grid.col [ Col.xs12, Col.mdAuto ]
                ...
                , button [ onClick Send, disabled (invalidInput model) ] [ text "Todo.." ]
                ]
            ]
        , Grid.row []
            [ Grid.col [ Col.xs12, Col.mdAuto ]
        ...
        , Grid.row []
            [ Grid.col [ Col.xs12, Col.mdAuto ]
                [ button [ onClick Send ] [ text "Update" ]
                ]
            ]
        , Grid.row []
            [ Grid.col [ Col.xs12, Col.mdAuto ]
